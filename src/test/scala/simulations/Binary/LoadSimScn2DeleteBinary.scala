...
class LoadSimScn2DeleteBinary extends Simulation {

  before {
    // to print the test config before the test execution
    println(s"::: Test Execution For Delete Binary With $LS2_USER_COUNT Virtual Users")
    println(s"::: Ramping Users Over $LS2_RAMP_DURATION Seconds")
    println(s"::: Total Test Duration: $LS2_TEST_DURATION Seconds")

    // to get objects ids that have to be deleted
    setUp(BinaryScn.createBinaryScn.inject(atOnceUsers(1)))
  }
...
//  setUp(BinaryScn.deleteOneBinaryScn.inject(rampUsers(LS2_USER_COUNT) during (LS2_RAMP_DURATION seconds)))
  setUp(BinaryScn.deleteOneBinaryScn.inject(atOnceUsers(1)))
