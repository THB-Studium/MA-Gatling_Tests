...
class LoadSimScn2ListAllBinaries extends Simulation {

  before {
    // to print the test config before the test execution
    println(s"::: Test Execution For Fetch All Binaries With $LS2_USER_COUNT Virtual Users")
  ...
  setUp(BinaryScn.listAllBinariesScn.inject(rampUsers(LS2_USER_COUNT) during (LS2_RAMP_DURATION seconds)))
